...
describe("ViewIssueComponent", () => {
  ...
  it("should render issue details", async () => {
    expect(fixture.debugElement.query(By.css(".summary-editor")).nativeElement.textContent).toEqual("testSummary");
    expect(fixture.debugElement.query(By.css(".description-editor")).nativeElement.textContent).toEqual(
      "testDescription"
    );
    const [prioritySelect, statusSelect] = await loader.getAllHarnesses(MatSelectHarness);
  ...
  it("should save the changed issue details on close button click", async () => {
    ...
    const [prioritySelect, statusSelect] = await loader.getAllHarnesses(MatSelectHarness);
    const [deleteButton, closeButton] = await loader.getAllHarnesses(MatButtonHarness);
